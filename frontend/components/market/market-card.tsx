...
export function MarketCard({ item }: MarketCardProps) {
  return (
    <article className="group flex h-[27rem] flex-col overflow-hidden rounded-md border border-neutral-200/80 bg-white shadow-[0_8px_24px_rgb(15_23_42_/_7%)] transition hover:-translate-y-0.5 hover:shadow-[0_14px_32px_rgb(15_23_42_/_10%)]">
      ...
      <div className="flex min-h-0 flex-1 flex-col p-4">
        <div className="flex min-h-[4.5rem] items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 className="mt-1 line-clamp-2 min-h-12 text-base font-semibold leading-6 text-neutral-950">
              {item.name}
            </h2>
          </div>
        ...
        <p className="mt-2 line-clamp-1 min-h-5 text-sm font-medium text-neutral-600">
          {item.seller}
        </p>
        <p className="mt-2 line-clamp-2 min-h-10 text-xs leading-5 text-neutral-500">
          {item.description}
        </p>
        <p className="mt-2 flex min-h-4 items-center gap-1.5 text-xs text-neutral-400">
        ...
        <div className="mt-auto flex min-h-10 items-center justify-between gap-3 pt-4">
          ...
          {item.googleMapsUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex h-9 items-center justify-center gap-1.5 rounded-md bg-neutral-950 px-3 text-xs font-semibold text-white transition hover:bg-neutral-800"
            >
              Map
